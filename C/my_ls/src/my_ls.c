/*
** my_ls.c for my_ls in /Users/habi_a/Documents/ETNA/C
**
** Made by HABI Açal
** Login   <habi_a@etna-alternance.net>
**
** Started on  Fri Apr  6 10:02:16 2018 HABI Açal
** Last update Thu Jul 12 17:43:21 2018 HABI Açal
*/
#include <check.h>
#include <flags.h>
#include <my_ls.h>
...
static unsigned int	get_nb_directory(char **resultab,
...
static void		recursivity(char **resultab, unsigned int result_size,
				    const char *pwd, t_list_flags *list_flags,
				    t_node_months **arr_months)
{
    ...
    if (i > 0)
    {
        ...
        if (get_flags('t', list_flags) && !get_flags('U', list_flags))
            sort_results_by_date(new_tols, new_size, list_flags,
				 arr_months, "./");
...
static void		print_total(char **resultab, unsigned int size,
				    const char *pwd, t_list_flags *list_flags)
{
    char		*full_path;
    unsigned int	i;
    ...
    while (i < size)
    {
	  ...
	  lstat(full_path, &buffer) : stat(full_path, &buffer);
        ...
        else if (my_strcmp(resultab[i], ".") && my_strcmp(resultab[i], "..")
...
static void		run(t_list_flags *list_flags, const char *path,
			    unsigned int argc, t_node_months **arr_months)
{
    char		**resultab;
    ...
    if (!get_flags('U', list_flags))
    {
        if (get_flags('t', list_flags) && list_results._size > 0)
            sort_results_by_date(resultab, list_results._size, list_flags,
				 arr_months, path);
    ...
    if (get_flags('R', list_flags))
        recursivity(resultab, list_results._size, path,
		    list_flags, arr_months);
